...
(defn ui []
  [:div.bg-gray-100.p-6.appearance-none
   [:div.p-8
    ...
    [panel
     {:title "Your Profile"
      :description "Your primary JUXT profile"}
     [user-details]]
    ...
    [panel
     {:title "Password"
      :description "Reset your password for this site"}
    ...
    [panel
     {:title "Access tokens"
      :description "Manage your Site API access tokens"}
     ]
    ...
    [panel
     {:title "Phone details"
      :description "If we ever need to ring you directly"}
     [input {:label "Your mobile"}]
     [button "Save"]]
    ...
    [panel
     {:title "Address"
      :description "Provide JUXT with your address"}
     [:p "We occasionally want to send you stuff through the post."]
     [input {:label "Building and street"}]
     [input {}]
     [input {:label "Town or City"}]
     [input {:label "County or Province"}]
    ...
    [panel
     {:title "Dietary"
      :description "Please indicate any dietary preferences"}
     [:p "Sometimes we'd like to send you food and drink, or book restaurants for team meals. This information helps us avoid getting it wrong."]
     [input {:label "Vegetarian" :type "checkbox"}]
     [input {:label "Vegan" :type "checkbox"}]
     [input {:label "Beer" :type "checkbox"}]
     [input {:label "Wine" :type "checkbox"}]
     [input {:label "Spirits" :type "checkbox"}]
     [input {:label "Allegies"}]
     [button "Save"]
     ]
    ...
    [panel
     {:title "Emergency details"
      :description "We will only use this information in an emergency"}
     [input {:label "Next of kin"}]
     [input {:label "Contact details (mobile)"}]
